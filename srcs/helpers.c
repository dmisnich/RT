...
void		find_normal_cyl(t_math *m, t_obj *o)
{
	double m_;
...
void		find_normal_par(t_math *m, t_obj *o)
{
	double m_;

	t_vector x = vector_sub(m->r.start, o->pos);
	m_ = vector_dot(vector_scale(m->t, o->dir), m->r.dir) + vector_dot(o->dir, x);
	m->norm = vector_sub(m->norm, vector_scale(m_ + o->radius, o->dir));
}

void		find_normal_cone(t_math *m, t_obj *o)
{
	double m_;
	double a;
...
void		find_normal_all(t_math *m, t_obj *o, t_env *e)
{
	...
	if (o->inter == intersect_cyl)
		...
		find_normal_cone(m, o);
	else if (o->inter == intersect_par)
		find_normal_par(m, o);
...
void		find_inter(t_math *m, t_env *e)
{
	...
	// while ((m->color.level < 10) && m->color.coef > 0.01f)
	// {
		m->t = 20000.0f;
		c = -1;
		i = -1;
		...
		if (c == -1)
			return ;
		find_normal_all(m, &e->objects[c], e);
	// }
}
...
void		find_shape(t_env *e, char *s)
{
	if (e->objects[e->i].inter == 0)
	{
		...
		else if (ft_strstr(s, "par"))
			e->objects[e->i].inter = intersect_par;
		else if (ft_strstr(s, "elips"))
			e->objects[e->i].inter = intersect_elips;
	}
}
